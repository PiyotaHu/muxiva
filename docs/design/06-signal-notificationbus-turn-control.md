# Muxiva Stage 6 Signal, NotificationBus, and Resource Contract
...
Stage 6 adds two control and observation scopes:

- a `SignalFrame` travels only from a node to the directly connected downstream
  nodes of its enabled graph edges; and
- a process-local notification travels through the graph-owned `NotificationBus` to
  subscribers of its exact topic, without using an Event output port or Edge.

The NotificationBus currently uses an immutable `EventFrame` as its typed envelope,
while an `EventFrame` emitted with `NodeContext::emit` remains ordinary Graph data.
Their `NamespacedName`, `SchemaVersion`, common header timestamp, source `NodeId`,
and closed `Value` payload are authoritative. There is no bare JSON, bare `Value`,
media-event, or implicit parameter channel.
...
## 3. Process-local NotificationBus

`NotificationBus` exposes exactly the core operations:
...
The concurrent runtime owns cloneable NotificationBus and ResourceStore handles. Stop
seals their admission immediately. The coordinator reaps NotificationBus workers and
...
- Runtime Stop is idempotent from any thread, closes media and Signal queues,
  ...
  finish/abort, then releases NotificationBus and resource ownership.
...
- NotificationBus metrics are per subscription snapshots but are not yet connected to
...
POD/vtables. It must not expose `ResourceStore`, NotificationBus handlers, Rust trait
