...
use voxa_core::{ManagedStreamAdapter, Node, NodeContext, NodeKind, PortName};
use voxa_testkit::{
...
fn graph_and_node_helpers_record_owned_deterministic_data() {
    ...
    let mut context = NodeContext::new(
        voxa_types::NodeId::new("node").unwrap(),
        voxa_core::ConfigMap::empty(),
        None,
    );
...
fn sink_frame_and_managed_helpers_are_copy_owned_and_scripted() {
    ...
    let mut context = NodeContext::new(
        voxa_types::NodeId::new("sink").unwrap(),
        voxa_core::ConfigMap::empty(),
        None,
    );
    ...
    let request = voxa_core::AdapterRequest {
        request_id: voxa_core::RequestId::new(9),
        session_id: voxa_types::SessionId::new("session").unwrap(),
        input: text_frame(9, "x"),
        attempt: 1,
    };
    assert!(matches!(
        adapter.send(request),
        voxa_core::AdapterResponse::Failed(_)
    ));
}
