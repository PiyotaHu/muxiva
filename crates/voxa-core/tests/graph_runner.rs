...
fn signal_frame(id: &str) -> Frame {
    frame(
        ...
        FramePayload::Signal(SignalData::new(
            NamespacedName::new("voxa.signal.test").unwrap(),
...
impl TestNode {
    ...
    fn checkpoint(&self, hook: Hook) -> voxa_types::Result<()> {
...
impl TestPolicy {
    ...
    fn checkpoint(&self, phase: &'static str) -> voxa_types::Result<()> {
...
impl EdgePolicy for TestPolicy {
    ...
    ) -> voxa_types::Result<ValidationDecision> {
...
fn validation_drop_skips_transform_and_explicit_abort_stops_delivery() {
    for (failure_action, expected_code) in [
        ...
        (ValidationFailureAction::Abort, Some("VOXA-RUN-VALIDATION")),
    ] {
    ...
    for action in [
        ...
        match action {
            ...
            PolicyAction::Abort(_) => {
                assert_eq!(result.unwrap_err().root().code(), "VOXA-RUN-POLICY-ABORT");
...
fn two_source_graph() -> voxa_core::GraphDefinition {
    let mut builder = GraphBuilder::new();
    for id in ["a", "b"] {
        builder
            .add_node(descriptor(
