...
use crate::{
    ...
    MediaTimeRange, Metadata, MuxivaError, Result, SequenceId, StreamId, Timestamp, TraceId,
    TransformOrigin,
};
...
fn arithmetic_error() -> crate::MuxivaError {
    crate::MuxivaError::new(
...
impl Frame {
    ...
    ///
    /// ```compile_fail
    /// # use muxiva_types::{AudioData, Frame, FrameId, TransformOrigin};
    ...
    pub fn merge_audio(
        ...
        if !(2..=MAX_MERGE_PARENTS).contains(&parents.len()) {
            return Err(MuxivaError::new(
                ErrorCategory::Validation,
                "MUXIVA-FRM-AUDIO-MERGE-PARENTS",
        ...
        for parent in parents {
            ...
            if next_lineage_len > 4_096 {
                return Err(MuxivaError::new(
                    ErrorCategory::Validation,
                    "MUXIVA-FRM-AUDIO-MERGE-LINEAGE",
...
fn audio_merge_type_error() -> MuxivaError {
    MuxivaError::new(
        ErrorCategory::Validation,
        "MUXIVA-FRM-AUDIO-MERGE-TYPE",
        "only audio frames can be merged",
    )
}

fn audio_merge_compatibility_error(message: &'static str) -> MuxivaError {
    MuxivaError::new(
        ErrorCategory::Validation,
        "MUXIVA-FRM-AUDIO-MERGE-COMPATIBILITY",
        message,
    )
}
...
fn type_mismatch_error(expected: FrameType, actual: FrameType) -> MuxivaError {
    MuxivaError::new(
        ErrorCategory::Validation,
