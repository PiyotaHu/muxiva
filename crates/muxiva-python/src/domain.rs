...
impl PythonNodeExecutionDomain {
    ...
    pub(crate) fn new(
        ...
        let nz = |value, name| {
            NonZeroUsize::new(value).ok_or_else(|| {
                binding_error("MUXIVA-PY-CAPACITY", format!("{name} must be non-zero"))
            })
        };
        ...
        let driver = ForeignNodeDriver::new(config)
            .map_err(|e| binding_error("MUXIVA-PY-CONFIG", e.to_string()))?;
        ...
        let handle = thread::Builder::new()
            .name("muxiva-python-node".into())
            .spawn(move || {
                run_domain(worker_driver, node, call_timeout, worker_output_ports);
                let _ = done_tx.send(());
            })
            .map_err(|e| binding_error("MUXIVA-PY-THREAD", e.to_string()))?;
        Ok(Self {
            driver,
    ...
    fn close(&self, py: Python<'_>) -> PyResult<bool> {
        ...
        if completed {
            ...
            Err(binding_error("MUXIVA-PY-SHUTDOWN-DEADLINE", "Python task did not stop before shutdown deadline; in-process thread cannot be killed"))
        }
    }
...
impl PythonNodeExecutionDomain {
    ...
    pub(crate) fn submit_blocking(
    ...
    ) -> muxiva_types::Result<ForeignCompletion> {
        {
            let state = self.state.lock().unwrap_or_else(|e| e.into_inner());
            if state.closed {
                return Err(domain_error("MUXIVA-PY-CLOSED", "Python domain is closed"));
            }
        }
        ...
        match self
            ...
            .map_err(|error| domain_error("MUXIVA-PY-DRIVER", error.to_string()))?
        {
            ForeignSubmitOutcome::Accepted => {
                *next = next
                    .checked_add(1)
                    .ok_or_else(|| domain_error("MUXIVA-PY-SEQUENCE", "sequence exhausted"))?;
            }
            ForeignSubmitOutcome::Full => {
                return Err(domain_error(
                    "MUXIVA-PY-INBOX-FULL",
    ...
    pub(crate) fn close_blocking(&self) -> muxiva_types::Result<bool> {
        let mut state = self.state.lock().unwrap_or_else(|e| e.into_inner());
        if state.closed {
            return Ok(false);
        }
    ...
    fn submit(&self, py: Python<'_>, kind: ForeignCommandKind) -> PyResult<ForeignCompletion> {
        {
            let state = self.state.lock().unwrap_or_else(|e| e.into_inner());
            if state.closed {
                return Err(binding_error("MUXIVA-PY-CLOSED", "Python domain is closed"));
            }
        }
        let mut next = self.sequence.lock().unwrap_or_else(|e| e.into_inner());
        let sequence = *next;
        match self
            .driver
            .try_submit(ForeignCommand::new(sequence, kind), Instant::now())
            .map_err(|e| binding_error("MUXIVA-PY-DRIVER", e.to_string()))?
        {
            ForeignSubmitOutcome::Accepted => {
                *next = next
                    .checked_add(1)
                    .ok_or_else(|| binding_error("MUXIVA-PY-SEQUENCE", "sequence exhausted"))?;
            }
            ForeignSubmitOutcome::Full => {
                return Err(binding_error(
                    "MUXIVA-PY-INBOX-FULL",
...
fn domain_error(code: &str, message: impl AsRef<str>) -> muxiva_types::MuxivaError {
    muxiva_types::MuxivaError::try_new(
        muxiva_types::ErrorCategory::External,
        code.to_owned(),
