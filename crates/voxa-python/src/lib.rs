#![forbid(unsafe_code)]
//! PyO3 boundary for owned Voxa values and bounded Python node execution.
...
fn _native(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add("VoxaError", module.py().get_type::<VoxaError>())?;
...
pub(crate) fn binding_error(code: &str, message: impl AsRef<str>) -> PyErr {
    VoxaError::new_err(format!("{code}: {}", message.as_ref()))
}
    ...
    fn isolated_process_is_rejected_explicitly() {
        Python::attach(|py| {
            ...
            let error = module
                ...
                .unwrap_err();
            assert!(error.to_string().contains("VOXA-PY-ISOLATION-UNSUPPORTED"));
        });
    }
}
