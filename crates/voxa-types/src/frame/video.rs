...
use crate::{ErrorCategory, FrameBuffer, Result, VoxaError};
...
fn dimensions_error() -> VoxaError {
    VoxaError::new(
        ErrorCategory::Validation,
        "VOXA-FRM-VIDEO-DIMENSIONS",
        "video dimensions are invalid for the pixel format",
    )
}

fn invalid_plane_error() -> VoxaError {
    VoxaError::new(
        ErrorCategory::Validation,
