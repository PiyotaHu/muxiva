...
build=${TMPDIR:-/tmp}/voxa-media-build
mkdir -p "$build"
cxx_bin=${CXX:-c++}
cxx_system=
...
"$cxx_bin" -std=c++17 -Wall -Wextra -Wpedantic -Werror -pthread $cxx_system \
  -I"$repo/cpp/media/include" \
  "$repo/cpp/media/src/media_pipeline.cc" \
  "$repo/cpp/media/tests/media_pipeline_test.cc" \
  -o "$build/media_pipeline_test"
"$build/media_pipeline_test"

ffmpeg_flags=
pkg_path=${VOXA_FFMPEG_PKG_CONFIG_PATH:-}
...
if [ -z "$ffmpeg_flags" ]; then
  echo "SKIP FFmpeg backend: libavutil, libswresample and libswscale pkg-config files are unavailable"
  exit 0
fi
"$cxx_bin" -std=c++17 -Wall -Wextra -Wpedantic -Werror -pthread $cxx_system \
  -DVOXA_ENABLE_FFMPEG=1 -I"$repo/cpp/media/include" \
  "$repo/cpp/media/src/media_pipeline.cc" "$repo/cpp/media/src/ffmpeg_backend.cc" \
  "$repo/cpp/media/tests/ffmpeg_backend_test.cc" \
  $ffmpeg_flags \
  -o "$build/ffmpeg_backend_test"
"$build/ffmpeg_backend_test"
