...
function dispatch(command) {
  ...
  if (!implementation) return JSON.stringify({ ok: false, error: { code: 'VOXA_NODE_GRAPH_FACTORY', message: `unknown TypeScript factory ${command.nodeType}` } })
...
try {
  ...
  parentPort.postMessage({ ok: false, error: { code: error.code ?? 'VOXA_NODE_GRAPH', message: String(error.message ?? error) } })
}
