...
public:
  ...
  int initialize(const std::string &app_id,
    ...
    try {
      return executor_.call([this, app_id, observer] {
        if (engine_ != nullptr || observer == nullptr)
          return -2;
        ...
        if (result != 0)
          return result;
        void *media = nullptr;
        result = engine_->queryInterface(::agora::rtc::AGORA_IID_MEDIA_ENGINE,
                                         &media);
        if (result != 0 || media == nullptr)
          return result == 0 ? -1 : result;
        media_ = static_cast<::agora::media::IMediaEngine *>(media);
        if ((result = media_->registerAudioFrameObserver(this)) != 0)
          return result;
        if ((result = media_->registerVideoFrameObserver(this)) != 0)
          return result;
        // Agora only activates the per-user before-mixing callback after its
        // output format is configured before joinChannel.
        if ((result = engine_->setPlaybackAudioFrameBeforeMixingParameters(
                 16000, 1)) != 0) {
          return result;
        }
  ...
  int leave() noexcept override {
    return executor_.call(
        [this] { return engine_ == nullptr ? 0 : engine_->leaveChannel(); });
  }
  ...
  int push_audio(const Pcm16FrameView &value) noexcept override {
    try {
      ...
      return executor_.call([this, bytes = std::move(bytes), value]() mutable {
        if (media_ == nullptr || value.sample_rate_hz != 48000 ||
            value.channels != 1) {
          return -2;
        }
...
private:
    ...
    if (observer != nullptr && frame.buffer != nullptr &&
      ...
      if (count == 1 || count % 500 == 0) {
        std::fprintf(stderr,
                     ...
                     frame.samplesPerSec, frame.channels);
      }
    }
    return true;
  }
  ...
  bool onRenderVideoFrame(const char *, ::agora::rtc::uid_t uid,
    ...
    if (observer != nullptr &&
      ...
      observer->on_video_frame({frame.yBuffer, frame.uBuffer, frame.vBuffer,
  ...
  void detach(SdkObserver *observer) noexcept {
    try {
      ...
      bool leave = false;
      {
        ...
        // invoking this observer, so detach cannot return into freed Node
        // state.
        std::lock_guard<std::recursive_mutex> lock(mutex_);
        observers_.erase(
...
private:
  ...
  void on_connection_state(ConnectionState state,
                           int reason) noexcept override {
    broadcast(
