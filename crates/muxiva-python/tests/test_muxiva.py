...
def test_six_immutable_owned_frames():
    frames = [
        muxiva.TextFrame("hello"),
        muxiva.ByteFrame(b"abc", media_type="application/octet-stream"),
        muxiva.AudioFrame(b"\0\0" * 160, 16_000, 1, 160),
        muxiva.VideoFrame(bytes(16), 2, 2),
        muxiva.SignalFrame("muxiva.test.signal", "payload"),
        muxiva.EventFrame("muxiva.test.event", "payload"),
    ]
...
def test_async_lifecycle_uses_one_private_loop_and_thread():
    ...
    domain.prepare()
    output = domain.process(muxiva.TextFrame("hello"))
...
def test_exception_deadline_and_isolated_process_rejection():
    ...
    domain = muxiva.PythonNodeExecutionDomain(Broken())
    with pytest.raises(muxiva.MuxivaError, match="MUXIVA-PY-EXCEPTION"):
        domain.process(muxiva.TextFrame("x"))

    with pytest.raises(muxiva.MuxivaError, match="MUXIVA-PY-ISOLATION-UNSUPPORTED"):
...
def test_high_level_node_runner_manages_lifecycle():
    ...
    node = Uppercase()
    with muxiva.NodeRunner(node) as runner:
        [output] = runner.process(muxiva.TextFrame("hello", sequence=7))
...
def test_node_runner_aborts_on_exceptional_context_exit():
    class Observed(muxiva.TransformNode):
        def __init__(self):
            self.reason = None
...
def test_python_factory_executes_inside_registered_graph_v1_runtime():
    ...
    class Uppercase:
        ...
        def on_process(self, frame):
            ...
            return muxiva.TextFrame(frame.text.upper())
    ...
    assert muxiva.run_graph(graph, [factory]) == 3
...
def test_schema_driven_multimodal_source_and_sinks():
    ...
    class Source:
        ...
        def on_process(self):
            return {
                "audio_out": muxiva.AudioFrame(b"\x00\x00", 8000, 1, 1, sequence=7),
                "video_out": muxiva.VideoFrame(b"\x01\x02\x03\x04", 1, 1, sequence=7),
                "byte_out": muxiva.ByteFrame(b"raw", media_type="application/octet-stream", sequence=7),
                "text_out": [muxiva.TextFrame("one", sequence=7), muxiva.TextFrame("two", sequence=8)],
            }
    ...
    assert muxiva.run_graph(graph, factories) == 5
