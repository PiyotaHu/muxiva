...
Voxa Runtime 只运行 Node，不存在另一种叫 Provider 的运行实体。Agora、Qwen、FFmpeg
...
| 项目 Node | `my_agent.rag` | 位于 Agent 的 `.voxa/nodes/`，由开发者维护 |

外部连接配置由 Node Manifest 的 `connection_id` 声明，并保存在项目 `.env`。Connection
只是凭据配置，不产生新的运行时抽象。

开发者可以复制官方 Node 的结构，在 `.voxa/nodes/<package>/` 中编写 Rust、Python、
