...
fn map_core<T, E: std::fmt::Display>(result: Result<T, E>) -> PyResult<T> {
    result.map_err(|error| binding_error("MUXIVA-PY-FRAME", error.to_string()))
}
...
impl PyByteFrame {
    ...
    fn new(
        ...
        let media_type = media_type
            .map(MediaType::new)
            .transpose()
            .map_err(|e| binding_error("MUXIVA-PY-FRAME", e.to_string()))?;
        ...
        let video = match pixel_format {
            ...
            "yuv420p" => {
                let s = strides.ok_or_else(|| {
                    binding_error("MUXIVA-PY-VIDEO-STRIDE", "yuv420p requires three strides")
                })?;
                if s.len() != 3 {
                    return Err(binding_error(
                        "MUXIVA-PY-VIDEO-STRIDE",
...
pub(crate) fn extract_frame(value: &Bound<'_, PyAny>) -> PyResult<Frame> {
    ...
    Err(binding_error(
        "MUXIVA-PY-OUTPUT",
        "node output must be a Muxiva Frame or a sequence of Frames",
    ))
}
