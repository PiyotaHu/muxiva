...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    print_graph(&graph);
    println!("[MUXIVA][INFO][runtime.started] mode=sync source_frames=2");
    ...
    println!("[MUXIVA][INFO][runtime.completed] status=success outputs=2");
    println!("[MUXIVA][RESULT] {output}");
    Ok(())
}

fn print_graph(graph: &GraphDefinition) {
    println!(
        "[MUXIVA][INFO][graph.loaded] id={} nodes={} edges={}",
        graph.graph_id(),
        graph.nodes().len(),
        graph.edges().len()
    );
...
fn text_graph() -> muxiva_core::GraphDefinition {
