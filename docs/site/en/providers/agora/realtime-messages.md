...
| Node | Port | Schema |
| --- | --- | --- |
| `agora.data_source` | `message_out` Byte output | `application/vnd.voxa.client-command+json`, max 1 KiB |
| `agora.data_sink` | `message_in` Byte input | `application/vnd.voxa.client-event+json`, max 1 KiB |

The data stream is reliable and ordered. Voxa paces outbound messages below Agora's 6 KiB/s
limit. `builtin.client_event_encoder` serializes `EventFrame` values as
`voxa.client-event/v1`; messages larger than one packet use `voxa.transport-fragment/v1` and are
