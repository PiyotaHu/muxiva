...
use muxiva_types::{
    ...
    FrameHeader, FrameId, FramePayload, FrameType, Lineage, Metadata, MuxivaError, NamespacedName,
    NodeId, SchemaVersion, SequenceId, SignalData, SignalFrame, StreamId, Timestamp, TraceId,
    Value,
};
...
fn signal(sequence: u64, source: &str) -> SignalFrame {
    Frame::new(
        ...
        FramePayload::Signal(SignalData::new(
            NamespacedName::new("muxiva.test.signal").unwrap(),
...
fn connected_graph() -> muxiva_core::GraphDefinition {
...
impl Node for SignalSink {
    ...
    fn on_signal(&mut self, signal: SignalFrame, _: &mut NodeContext) -> muxiva_types::Result<()> {
        self.received
            .lock()
            .unwrap()
...
fn emitting_without_an_actual_edge_returns_a_structured_error() {
    ...
    let error: MuxivaError = error.into();
    assert_eq!(error.code(), "MUXIVA-SIGNAL-NO-EDGE");
}
...
fn event_bus_subscribe_unsubscribe_slow_and_faulty_handlers_are_isolated() {
    ...
    let reports = (0..20)
        .map(|sequence| bus.publish(event(sequence, "muxiva.test.events")).unwrap())
        .collect::<Vec<_>>();
    ...
    assert!(!bus.unsubscribe(fast));
    assert_eq!(
        bus.publish(event(100, "muxiva.test.events"))
            .unwrap()
            .matched,
        2
    );
    ...
    assert!(bus.publish(event(101, "muxiva.test.events")).is_err());
}
...
fn event_bus_stop_races_publish_without_panics_or_blocking() {
    ...
    let publisher = thread::spawn(move || {
        for sequence in 0..2_000 {
            if publisher_bus
                .publish(event(sequence, "muxiva.test.race"))
                .is_err()
            {
                break;
            }
