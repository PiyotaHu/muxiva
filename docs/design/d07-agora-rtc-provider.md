...
D07 adds optional Agora Nodes without making a proprietary SDK a Muxiva Core dependency. The C++
implementation translates Agora 4.x raw PCM16/I420 callbacks into bounded Muxiva ingress and egress.
...
- Video is tightly packed I420 at the Muxiva ABI and stride-aware I420 at the
...
- Connection, participant, and error callbacks become Muxiva Signal/Event frames.
...
`MUXIVA_ENABLE_AGORA=OFF`, which builds and tests the public adapter contract with
no vendor files. Enabling it requires an independently obtained Agora Native SDK
root and links the vendor library into `MuxivaAgora::agora`.
