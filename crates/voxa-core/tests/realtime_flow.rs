...
fn merge_leaves_mismatched_and_non_audio_prefixes_atomic() {
    ...
    discontinuous[0] = Frame::new(
        text_header,
        FramePayload::Text(voxa_types::TextData::new("not audio")),
    )
    .unwrap();
    assert!(merge_audio_prefix(
        &discontinuous,
...
fn frame_byte_and_media_limits_bind_independently_and_resume_is_hysteretic() {
    ...
    for dropped in 1..=8 {
        let update = flow
            .record_drop(audio, voxa_core::FlowDropReason::PressureShedding)
            .unwrap();
...
fn lossless_profiles_never_select_silent_loss_and_ports_are_independent() {
    ...
    assert_eq!(
        left.record_drop(audio, voxa_core::FlowDropReason::PressureShedding),
        Err(voxa_core::FlowError::LosslessDropForbidden)
    );
    for _ in 1..8 {
        left.record_enqueue(audio).unwrap();
    }
