...
use muxiva_types::{EdgeId, FiniteF64, FrameType, GraphId, NodeId, Value, ValueMap};
    ...
    registry
        ...
        .map(|registration| {
            ...
            NodeCatalogEntry {
                ...
                documentation: "https://piyotahu.github.io/muxiva/en/providers/builtin/".to_owned(),
...
pub fn parse(input: &str) -> Result<GraphDocument, Vec<GraphDiagnostic>> {
    ...
    let mut document: GraphDocument = serde_json::from_str(input)
        .map_err(|error| vec![diag("MUXIVA-GRAPH-JSON", &error.to_string(), "")])?;
    migrate_legacy_node_names(&mut document);
    if document.version != "muxiva.graph/v1" {
        return Err(vec![diag(
            "MUXIVA-GRAPH-VERSION",
            "expected muxiva.graph/v1",
            "/version",
        )]);
    }
...
/// Returns the stable Node type for names written by older Muxiva releases.
...
/// Compiles against the trusted built-ins shipped with Muxiva.
...
pub fn compile_with_registry(
    ...
    let graph_id = GraphId::new(document.graph_id.clone())
        .map_err(|error| vec![diag("MUXIVA-GRAPH-ID", &error.to_string(), "/graph_id")])?;
    ...
    if errors.is_empty() {
        builder
            .build()
            .map_err(|error| vec![diag("MUXIVA-GRAPH-BUILD", &error.to_string(), "")])
    } else {
        Err(errors)
    }
}
...
fn compile_node(
    ...
    let node_id = NodeId::new(node.id.clone()).map_err(|error| {
        diag(
            "MUXIVA-GRAPH-NODE-ID",
            &error.to_string(),
            &format!("{pointer}/id"),
        )
    })?;
    let node_type = muxiva_core::NodeTypeName::new(node.node_type.clone()).map_err(|error| {
        diag(
            "MUXIVA-GRAPH-NODE-TYPE",
            &error.to_string(),
    ...
    let descriptor = registry
        ...
        .map_err(|error| diag("MUXIVA-GRAPH-REGISTRY", &error.to_string(), &pointer))?;
    registry
        .validate_config(
            &node_type,
            language,
    ...
    builder
        .add_node(descriptor)
        .map_err(|error| diag("MUXIVA-GRAPH-NODE", &error.to_string(), &pointer))?;
    builder
...
/// Converts a deterministic Muxiva configuration map into a JSON object.
...
fn edge_descriptor(edge: &EdgeDocument) -> Result<EdgeDescriptor, String> {
    ...
    Ok(EdgeDescriptor::new(
        ...
        muxiva_core::PortName::new(edge.from.port.clone()).map_err(|error| error.to_string())?,
        NodeId::new(edge.to.node_id.clone()).map_err(|error| error.to_string())?,
        muxiva_core::PortName::new(edge.to.port.clone()).map_err(|error| error.to_string())?,
        frame_type,
