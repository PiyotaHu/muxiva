...
fn main() -> ExitCode {
    ...
    if !matches!(
        ...
        eprintln!("usage: muxiva-bench <queue|flow|frame-copy|managed-stream|stop>");
        return ExitCode::FAILURE;
    }

    let iterations = env_u64("MUXIVA_BENCH_ITERATIONS", 10_000).max(1);
    let warmup = env_u64("MUXIVA_BENCH_WARMUP", 1_000);
    let payload_bytes = env_u64("MUXIVA_BENCH_PAYLOAD_BYTES", 4096) as usize;
    ...
    let report = Report {
        ...
        scenario: &scenario,
        commit: env::var("MUXIVA_BENCH_COMMIT").unwrap_or_else(|_| "unknown".into()),
        os: env::consts::OS,
        arch: env::consts::ARCH,
        toolchain: env::var("MUXIVA_BENCH_TOOLCHAIN").unwrap_or_else(|_| "unknown".into()),
        iterations,
