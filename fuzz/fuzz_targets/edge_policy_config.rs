...
fn serde_json_text(overflow: &str) -> String {
    let escaped = overflow
        ...
        .collect::<String>();
    format!(
        r#"{{"version":"voxa.graph/v1","graph_id":"fuzz","nodes":[],"edges":[{{"id":"e","from":{{"node_id":"a","port":"out"}},"to":{{"node_id":"b","port":"in"}},"frame_type":"text","queue_policy":{{"capacity":1,"overflow":"{escaped}"}}}}]}}"#
    )
}
