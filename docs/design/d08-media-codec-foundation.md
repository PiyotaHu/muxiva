...
D08 adds `Voxa::media`, an optional C++17 media-normalization layer. The public
pipeline builds without FFmpeg; `VOXA_ENABLE_FFMPEG=ON` supplies the real
libswresample/libswscale backend. FFmpeg remains outside Voxa Core and outside
the default dependency graph.

## V1 formats

- Packed/interleaved PCM: U8, I16LE, I32LE, F32LE and F64LE through FFmpeg.
- I24LE remains part of the Voxa frame contract but the FFmpeg backend rejects
...
does not invent comparability across Voxa clock domains and does not implement
