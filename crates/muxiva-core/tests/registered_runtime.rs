...
use muxiva_types::{ErrorCategory, Frame, GraphId, MuxivaError, NodeId};
...
impl Node for TestSource {
    fn on_process(
        ...
        match self.behavior {
            Behavior::Success => Ok(()),
            Behavior::Abort => Err(MuxivaError::new(
...
fn graph(with_selection: bool) -> muxiva_core::GraphDefinition {
    let id = node_id();
...
fn registered_node_abort_reaches_the_runtime_terminal_result() {
    ...
    assert_eq!(reason.root().code(), "MUXIVA-TEST-ABORT");
    let metrics = runtime.node_metrics(&node_id()).unwrap();
    assert_eq!(metrics.process_total(), 1);
