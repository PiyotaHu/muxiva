...
use muxiva_types::{ErrorCategory, Frame, FrameType, MuxivaError, NodeId, SignalFrame};
use pyo3::prelude::*;
...
impl PyGraphNodeFactory {
    ...
    fn new(
        ...
        let schema_json: serde_json::Value = serde_json::from_str(config_schema_json)
            .map_err(|error| binding_error("MUXIVA-PY-GRAPH-SCHEMA", error.to_string()))?;
        Ok(Self {
            node_type: NodeTypeName::new(node_type)
                .map_err(|error| binding_error("MUXIVA-PY-GRAPH-NODE-TYPE", error.to_string()))?,
            version: NodeFactoryVersion::new(version).map_err(|error| {
                binding_error("MUXIVA-PY-GRAPH-FACTORY-VERSION", error.to_string())
            })?,
            kind,
            ports,
            config_schema: ConfigSchema::new(
                muxiva_graph_json::value_from_json(&schema_json)
                    .map_err(|error| binding_error("MUXIVA-PY-GRAPH-SCHEMA", error))?,
            ),
            pass_config,
            constructor,
        })
    ...
    fn create(
        ...
        let node = Python::attach(|py| {
            if self.pass_config {
                ...
                let encoded = muxiva_graph_json::config_map_to_json(config).to_string();
        ...
        .map_err(|error| NodeFactoryError::new("MUXIVA-PY-GRAPH-CONSTRUCTOR", error.to_string()))?;
        let domain = PythonNodeExecutionDomain::new_graph(node, self.output_ports.clone())
            .map_err(|error| NodeFactoryError::new("MUXIVA-PY-GRAPH-DOMAIN", error.to_string()))?;
...
impl ForeignNodeInstance for PythonInstance {
        ...
        let command = match input {
            Some(frame) => ForeignCommandKind::ProcessOnPort {
                ...
                input_port: input_port.cloned().ok_or_else(|| {
                    graph_error(
                        "MUXIVA-PY-GRAPH-INPUT-PORT",
                        "input port identity is missing",
                    )
                })?,
            },
    ...
    fn on_signal(&mut self, signal: SignalFrame) -> Result<ForeignNodeCallOutput, MuxivaError> {
        self.domain
...
pub fn run_graph(
    ...
    for factory in factories {
        registry
            ...
            .map_err(|error| binding_error("MUXIVA-PY-GRAPH-REGISTRY", error.to_string()))?;
    }
    let document = muxiva_graph_json::parse(graph_json).map_err(graph_diagnostics)?;
    let graph = muxiva_graph_json::compile_with_registry(&document, &registry)
        .map_err(graph_diagnostics)?;
    ...
    .map_err(|error| binding_error("MUXIVA-PY-GRAPH-START", error.to_string()))?;
...
fn graph_diagnostics(diagnostics: Vec<muxiva_graph_json::GraphDiagnostic>) -> PyErr {
    binding_error(
        "MUXIVA-PY-GRAPH-COMPILE",
        diagnostics
            .into_iter()
...
fn graph_error(code: &str, message: &str) -> MuxivaError {
    MuxivaError::new(ErrorCategory::External, code, message)
}
    ...
    Ok(vec![
        FactoryPort {
            name: PortName::new(input)
                .map_err(|error| binding_error("MUXIVA-PY-GRAPH-PORT", error.to_string()))?,
...
fn parse_ports(encoded: &str) -> PyResult<Vec<FactoryPort>> {
    let documents: Vec<FactoryPortDocument> = serde_json::from_str(encoded)
        .map_err(|error| binding_error("MUXIVA-PY-GRAPH-PORTS", error.to_string()))?;
    documents
        .into_iter()
        .map(|document| {
            let direction = match document.direction.as_str() {
                ...
                _ => {
                    return Err(binding_error(
                        "MUXIVA-PY-GRAPH-PORT-DIRECTION",
