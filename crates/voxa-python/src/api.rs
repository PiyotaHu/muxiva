...
use pyo3::prelude::*;
use voxa_core::EventBus;
use voxa_types::{Frame, NamespacedName};
...
impl PyRuntime {
    ...
    fn session(&self) -> PyResult<PySession> {
        if self.closed.load(Ordering::Acquire) {
            return Err(binding_error("VOXA-PY-CLOSED", "runtime is closed"));
        }
        Ok(PySession {
            id: format!(
                "python-session-{}",
...
impl PyEventBus {
    ...
    fn new(capacity: usize) -> PyResult<Self> {
        let capacity = NonZeroUsize::new(capacity)
            .ok_or_else(|| binding_error("VOXA-PY-CAPACITY", "capacity must be non-zero"))?;
        Ok(Self {
    ...
    fn publish(&self, event: PyRef<'_, PyEventFrame>) -> PyResult<(usize, usize, usize)> {
        ...
        let report = self
            .inner
            .publish(event)
            .map_err(|e| binding_error("VOXA-PY-EVENT-BUS", e.to_string()))?;
    ...
    fn subscribe(
        ...
        let topic = NamespacedName::new(topic)
            .map_err(|e| binding_error("VOXA-PY-EVENT-TOPIC", e.to_string()))?;
