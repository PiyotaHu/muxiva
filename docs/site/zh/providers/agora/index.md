...
Engine，将凭据隔离在 Graph JSON 之外，并与 Muxiva 交换类型化 PCM Audio Frame 和有界、
...
在 Studio **Connections** 中配置 App ID、频道、Muxiva Bot UID/Token 和浏览器
UID/Token。开发环境使用短期 Token，生产环境必须使用 Token Server。

首次运行不要只生成一个 Token：浏览器固定使用数字 UID `1001`，Muxiva Bot 使用 `2001`，
需要为同一个 Channel 分别生成两个 Token。App Certificate 只用于生成 Token，不进入 Muxiva。
