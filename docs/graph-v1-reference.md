...
Graph v1 is Muxiva's strict, pure-data graph protocol. The root requires
`version` (`muxiva.graph/v1`), `graph_id`, `nodes`, and `edges`. Unknown fields
are rejected and the complete document is limited to 1 MiB.

The machine-readable JSON Schema is bundled at
`crates/muxiva-graph-json/schema/graph-v1.schema.json` and served by Studio at
...
converted into Muxiva's closed `Value` algebra, validated before Node creation,
...
Muxiva currently ships these exact Rust registrations at Factory version
`1.0.0`:
...
Run `muxiva validate` before `muxiva run` or saving from Studio. Diagnostics carry
a stable code and JSON Pointer such as `/nodes/0/node_config`.

## Validation and execution

`muxiva validate` parses and compiles against the Registry without allocating a
Node or invoking a lifecycle callback. `muxiva run` performs the same compilation,
...
SDK hosts can register exact-version schema-driven Source, Transform, and Sink factories with
`muxiva.GraphNodeFactory`/`muxiva.run_graph`, `GraphNodeFactory`/`runGraph`, or
`muxiva::MultimodalGraphNodeFactory`/`Runtime::run_multimodal_graph` (the C++ text
compatibility API remains available). Ports are exactly typed as audio, video,
text, or byte and lifecycle results preserve every output port name. Graph files never load code or
